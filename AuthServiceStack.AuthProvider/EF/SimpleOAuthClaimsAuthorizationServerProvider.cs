...
namespace AuthServiceStack.AuthProvider.EF
{
    public class SimpleOAuthClaimsAuthorizationServerProvider : Microsoft.Owin.Security.OAuth.OAuthAuthorizationServerProvider
    {
        ...
        //private Dictionary<string,string> ClaimsTrans;
        private getUserClaims getUserClaims;
        //private getClaimsTrans getClaimsTrans;

        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="authRepository">鉴权依据仓库</param>
        ...
        public SimpleOAuthClaimsAuthorizationServerProvider(AuthServiceStack.AuthModel.EF.AuthWithClaimsRepository authRepository, bool userClientAuth = false, string AnoymouseAllowedOrigins = "*")
        {
            ...
            this.userClientAuth = userClientAuth;
            this.AnoymouseAllowedOrigins = AnoymouseAllowedOrigins;

            this.getUserClaims = (string username) => { return authRepository.getUserClaims(username); };
        ...
        public override async System.Threading.Tasks.Task ValidateClientAuthentication(Microsoft.Owin.Security.OAuth.OAuthValidateClientAuthenticationContext context)
        {
            ...
            var client = default(AuthServiceStack.AuthModel.EF.Models.Client);
        ...
        public override async System.Threading.Tasks.Task GrantResourceOwnerCredentials(Microsoft.Owin.Security.OAuth.OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            //var claims = authRepository.getUserClaims(this.ClaimsTrans, context.UserName);
            var claims = this.getUserClaims(context.UserName);
