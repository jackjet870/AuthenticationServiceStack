...
namespace MallAuth.Shared
{
    public static class confs
    {
       //public static string authConnectionConfig = ConfigurationManager.AppSettings["AuthConnectionConfig"];
    }

    public static class MallClaimsDefinition
    {
    ...
    public class GlobalCache
    {
        ...
        private void InitUserClaims()
        {
            ...
            using (var db = new DAL.AppAuthContext())
            {
                foreach (var uc in db.CustomUserClaims.Where(s => s.valid == true))
                {
                    if (MallClaimsDefinition.dict.ContainsKey(uc.ClaimType))
                    {
                        var newClaim = new Claim(MallClaimsDefinition.dict[uc.ClaimType], uc.ClaimValue);
